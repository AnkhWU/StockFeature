...
class StockFeature:
    ...
    def __init__(self, queue_bars, queue_results):
        ...
        self.dequeQuotaWave = AsyncDefaultdict(partial(deque, maxlen=1200))  # 存放QuotaDataAndWave对象的双端队列
        ...
        self.dequePriceL = AsyncDefaultdict(partial(deque, maxlen=30))
        self.dequeValue = AsyncDefaultdict(partial(deque, maxlen=30))
    ...
    async def getWave(self, TICKER: str) -> None:
        ...
        try:
            if lastHighLevelTime != -1 and lastLowLevelTime != -1:
                if (lastHighLevelTime > lastLowLevelTime) \
                        and ((pb_deque[-1].timestamp - lastHighLevelTime) < self.waveInterval)\
                    ...
                    await self.__modifyBackUp(TICKER)
                        ...
                        and ((pb_deque[-1].timestamp - lastLowLevelTime) < self.waveInterval)\
                        and (self.dequeQuotaWave[TICKER][-1].state != WaveType.DOWN):
                    await self.__modifyBackDown(TICKER)

            elif lastHighLevelTime != -1:
                if ((pb_deque[-1].timestamp - lastHighLevelTime) < self.waveInterval) \
                        and (self.dequeQuotaWave[TICKER][-1].state != WaveType.UP):
                    await self.__modifyBackUp(TICKER)

            elif lastLowLevelTime != -1:
                if ((pb_deque[-1].timestamp - lastLowLevelTime) < self.waveInterval) \
                        and (self.dequeQuotaWave[TICKER][-1].state != WaveType.DOWN):
                    await self.__modifyBackDown(TICKER)
            ...
            # 涨停情况时
                ...
                self.dequeQuotaWave[TICKER].append(
                    StateRecord(timestamp=current_timestamp,
                                order_book_id=TICKER,
                                ticker=TICKER,
                                lastprice=lastDequeQuotaWave.waveStartPrice,
                                state=WaveType.DOWN,
                                waveStartPrice=lastDequeQuotaWave.waveStartPrice,
                                value=(current_tick.new_price / previous_tick.new_price - 1)
                                      + lastDequeQuotaWave.value,
    ...
    async def __modifyBackUp(self, TICKER) -> None:
        ...
        if self.dequeQuotaWave[TICKER][-1].lastHighLevelPrice and \
                self.dequeQuotaWave[TICKER][-1].lastprice > self.dequeQuotaWave[TICKER][-1].lastHighLevelPrice:
            tickerQuotaWave = copy.copy(self.dequeQuotaWave[TICKER])
            target_wave = \
    ...
    async def __modifyBackDown(self, TICKER) -> None:
        ...
        if self.dequeQuotaWave[TICKER][-1].lastLowLevelPrice \
                and self.dequeQuotaWave[TICKER][-1].lastprice <= self.dequeQuotaWave[TICKER][-1].lastLowLevelPrice:
