...
class StockFeature:
    ...
    def __init__(self, queue_bars, queue_results, mix_components):
        self.mixComponents = mix_components
        self.target_indices = {'000001.SH', '000016.SH', '000300.SH', '000688.SH', '000905.SH'}
    ...
    async def __modifyBackUp(self, TICKER) -> None:
        ...
        len_before = len(self.dequeLast2Wave[TICKER])
        if len_before != 0:
            before_laststate = self.dequeLast2Wave[TICKER][-1].state
        else:
            before_laststate = 0
        self.dequeLast2Wave[TICKER].extend(self.dequeLast1Wave[TICKER])
        self.dequeLast2Wave[TICKER].extend(self.dequeCurrentWave[TICKER])
        self.dequeLast1Wave[TICKER].clear()
        self.dequeCurrentWave[TICKER].clear()

        tickerQuotaWave = self.dequeLast2Wave[TICKER]

        target_wave = \
            list(filter(lambda p: (p.timestamp == tickerQuotaWave[-1].lastHighLevelTime
                                   or p.lastHighLevelTime == tickerQuotaWave[-1].lastHighLevelTime
                                   or p.highLowTime == tickerQuotaWave[-1].lastHighLevelTime)
                                  and (p.value == tickerQuotaWave[-1].lastHighLevelValue
                                       or p.lastHighLevelValue == tickerQuotaWave[-1].lastHighLevelValue
                                       or p.highLowValue == tickerQuotaWave[-1].lastHighLevelValue),
                        tickerQuotaWave))[0]
        # target_wave = \
        ...
        for j in range(start_idx + 1, len(tickerQuotaWave)):
            ...
            self.dequeLast2Wave[TICKER][j].lastprice = 0.0
            await self.queueResults.put(self.dequeLast2Wave[TICKER][j])
            self.dequeLast2Wave[TICKER][j] = \
        ...
        self.dequeCurrentWave[TICKER].extend(self.dequeLast2Wave[TICKER])
        self.dequeLast2Wave[TICKER].clear()
        if before_laststate == "WAVELESS":
            counter = 0
            while counter < len_before:
                counter += 1
                self.dequeLast1Wave[TICKER].appendleft(self.dequeCurrentWave[TICKER].popleft())
    ...
    def __getThreshold(self, TICKER, preclose) -> None:
        ...
        try:
            if TICKER == "399006.SZ":
                self.threshold_1 = 0.001
                self.threshold_end = 0.001
                self.threshold_2 = 0.0008
